...
def predict_endpoint():
    ...
    received_data_df = pd.DataFrame(
        ...
        index=[0])
    MODEL_PATH = os.path.join('20260506-082606.joblib')
    print(MODEL_PATH)
    var_name = request.args.get('name')
    prepared_feat = prepare_features(received_data_df, False)

    prediction = predict(prepared_feat, MODEL_PATH)['predictions'].to_dict()
    print(var_name)
